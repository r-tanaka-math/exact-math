...
export function auditOutput(root,{staging=false}={}){let pdfs=0;for(const file of files(root)){const rel=path.relative(root,file).split(path.sep).join('/'),b=fs.readFileSync(file),r=reports.find(r=>r.logical_path===rel);if(siteIconFiles.has(rel)){if(digest(b)!==siteIconSha256[rel])throw Error('Favicon identity '+rel);continue}if(r){if(b.length!==r.bytes||digest(b)!==r.sha256)throw Error('PDF identity');pdfs++;continue}
 const privateRelease=process.env.EXACT_PROFILE==='PRIVATE_LOCAL_INTEGRATION'&&/^mathlibannex\/releases\/(?:v0\.3\.0|current-candidate)\/index\.html$/.test(rel);
 const allowed=staging?['robots.txt','research/sphere-rigidity/citation.json'].includes(rel):privateRelease||/^(?:index\.html|404\.html|(?:research|research\/(?:sr|sphere-rigidity|naimark-problem)|about|verification|mathlibannex|mathlibannex\/releases\/v0\.[12]\.0|mathlibannex\/projects\/(?:sphere-rigidity|mankiewicz)|corrections|licensing)\/index\.html|corrections\/(?:received|demo)\/index\.html|research\/sphere-rigidity\/citation\.json|release-state\.json|licensing\/boundary\.json|robots\.txt|_astro\/[a-zA-Z0-9_.-]+\.(?:js|css))$/.test(rel);
