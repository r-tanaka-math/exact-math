...
const dir=path.join(root,'publication/full-hp');
const read=p=>JSON.parse(fs.readFileSync(p,'utf8'));
const sha=b=>createHash('sha256').update(b).digest('hex');
const need=(v,m)=>{if(!v)throw Error('FULL_HP_PUBLIC_REFUSED: '+m)};
...
export function verifyFullHPSelection(){
 ...
 need(metadata.batch==='EM_FULL_HP_PUBLICATION_R1'&&metadata.public_cards===44,'batch/card count');
 same(metadata.project_card_counts,{mankiewicz:11,'sphere-rigidity':0,rosenberg:33,'naimark-problem':0},'four selected Projects');
 need(metadata.card_identities.length===44&&new Set(metadata.card_identities.map(c=>c.stable_card_id)).size===44,'44 unique approved Cards');
 const seen=new Set();
 ...
 const cards=selection.files.filter(r=>/^mathlibannex\/cards\/[a-f0-9]{64}\/card.json$/.test(r.path));need(cards.length===44,'44 exact Card projections');
 for(const row of cards){const c=read(path.join(selected,row.path)),binding=metadata.card_identities.find(x=>x.stable_card_id===c.stable_card_id);need(binding,'selected Card identity');same(c.card_ref,binding.card_ref,'canonical Card ref');need(c.website_metadata?.catalog_current===true&&c.website_metadata?.approval_scope==='APPROVED_REUSED_SOURCE_EXPOSITION_CORRESPONDENCE','approval inheritance');}
 const catalog=read(path.join(selected,'mathlibannex/catalog/current.json'));need(catalog.card_count===44&&catalog.state==='PUBLIC_CURRENT','Catalog current');
...
export function validateFullHPAct(act,identity,deployment,today){
 const {metadata,selection_sha256,metadata_sha256}=verifyFullHPSelection();
 need(act?.schema==='exact.owner-postpublic-update-act.v4'&&act.authorized===true&&act.authorization_state==='OWNER_AUTHORIZED_FOR_EXACT_UPDATE'&&act.owner==='Ryotaro Tanaka','explicit exact owner act');
 need(act.update_kind==='EM_FULL_HP_PUBLICATION_R1','update kind');
 ...
 same(act.card_identities,metadata.card_identities,'all 44 approved Card identities');
 ...
 const manifestPath='publication/full-hp/source-files.json',manifestBytes=fs.readFileSync(path.join(root,manifestPath));
...
export function integrateFullHP(output,base){
 const {selection,selected}=verifyFullHPSelection();
 for(const row of selection.files){
 ...
 console.log('PASS_FULL_HP_PUBLIC_SELECTION',selection.files.length,base);
}
export function finishFullHPOutput(output,deployment,profile,mode,release){
 const {metadata,selection_sha256,metadata_sha256}=verifyFullHPSelection();const identity=gitIdentity(profile),{base,origin}=deployment;
 ...
 const state={schema:'exact.site-release-state.v6',site_commit:identity.commit,site_tree:identity.tree,public_parent_commit:metadata.old_commit,profile,canonical_origin:origin,base,deployment_profile:deployment.deployment_profile,public_repository:deployment.public_repository,publication_state:release?'AUTHORIZED_POSTPUBLIC_UPDATE':'LOCAL_POSTPUBLIC_SUCCESSOR_QUALIFICATION',publication_batch:metadata.batch,publication_date:metadata.publication_date,source:'PUBLISHED_V0_4_0',source_commit:metadata.source_release.commit,source_tree:metadata.source_release.tree,public_cards:44,catalog_current:'MANKIEWICZ_11_ROSENBERG_33_PUBLIC',project_card_counts:metadata.project_card_counts,project_nodes:metadata.project_nodes,approval_scope:'APPROVED_REUSED_SOURCE_EXPOSITION_CORRESPONDENCE',new_mathematical_review:false,content_terms:'PUBLIC_EFFECTIVE',content_terms_effective_date:'2026-09-17',site_first_publication_date:'2026-09-17',brief_first_publication_date:'2026-09-17',reports:metadata.reports,selection_sha256,metadata_sha256,indexing:release?'PUBLIC_INDEXABLE':'LOCAL_INDEXABLE_QUALIFICATION',feedback_mode:mode,formspree_automated_live_posts:0};
 fs.writeFileSync(path.join(output,'release-state.json'),JSON.stringify(state,null,2)+'\n');
 console.log('PASS_FULL_HP_PUBLIC_OUTPUT',files(output).length,profile);
}
